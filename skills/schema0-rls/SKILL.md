...
```bash
schema0 sandbox exec "bun run scaffold-scripts/generate.ts rls-service products"
```

Generated output:

| Output File | Location                             | Responsibility                                                               |
| ----------- | ------------------------------------ | ---------------------------------------------------------------------------- |
| RLS Router  | `packages/api/src/routers/[name].ts` | ORPC router with `protectedProcedure`, `createRLSTransaction`, CRUD with RLS |

The template generates: `getAll`, `getById`, `create`, `update`, `delete` — all using `protectedProcedure` and `createRLSTransaction`.
